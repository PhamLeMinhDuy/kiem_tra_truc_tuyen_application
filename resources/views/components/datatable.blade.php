        ...
        <tbody>
            @foreach ($danhSachDuLieu as $duLieu)
                <tr id="row-{{ $duLieu['id'] }}" class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                    ...
                    @else
                        <td class="flex items-center px-6 py-4">
                            <a href="{{ route('giang-vien.quan-ly.xem-diem.bang-diem-sinh-vien-giang-vien', [$giangVien->id, $duLieu['ma_lop_hoc_phan'], $duLieu['ma_bai_thi']]) }}" class="border border-green-500 text-black px-4 py-2 rounded-md bg-white transition-colors duration-300 hover:text-white hover:bg-green-500">Xem điểm</a>
                        </td>
                    @endif
                </tr>
            @endforeach
